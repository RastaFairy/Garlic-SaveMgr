...
public sealed class PayloadLauncherService
{
    ...
    private static readonly TimeSpan GitHubRateLimitBackoff = TimeSpan.FromMinutes(15);
    private const string GitHubRateLimitCacheFile = "github-rate-limit.txt";
        ...
        CancellationToken ct = default)
    {
        ...
        var hasUsableCache = File.Exists(elfPath);

        // El arranque nunca debe esperar a una API externa si ya dispone de un payload válido.
        if (hasUsableCache)
        {
            log?.Invoke($"Payload cacheado disponible ({cached?.TagName ?? "versión desconocida"}); se omite la espera de red durante el arranque.", "info");
            if (cached is not null)
                _ = RefreshPayloadCacheSilentlyAsync(log);
            return (cached?.TagName, true, cached?.Sha256);
        }

        try
        {
            var release = await FetchLatestReleaseAsync(ct).ConfigureAwait(false);
            if (release is null) return (null, false, null);
            var path = await EnsureElfAsync(release, log, null, ct).ConfigureAwait(false);
            var meta = LoadCachedMeta(metaPath);
    ...
    private async Task RefreshPayloadCacheSilentlyAsync(Action<string, string>? log)
    {
        try
        {
            using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
            var release = await FetchLatestReleaseAsync(cts.Token).ConfigureAwait(false);
            if (release is null) return;
            var current = LoadCachedMeta(Path.Combine(_cacheDir, MetaCacheFile));
            if (current is not null && string.Equals(current.TagName, release.TagName, StringComparison.OrdinalIgnoreCase) &&
                File.Exists(Path.Combine(_cacheDir, ElfCacheFile)))
            {
                log?.Invoke($"Payload remoto comprobado en segundo plano: {release.TagName}.", "info");
                return;
            }
            await EnsureElfAsync(release, log, null, cts.Token).ConfigureAwait(false);
        }
        catch (OperationCanceledException) { }
        catch (Exception ex) { LogService.Write($"Payload background refresh omitido: {ex.Message}", "INFO"); }
    }
    ...
    private async Task<ReleaseInfo?> FetchLatestReleaseAsync(CancellationToken ct)
    {
        ...
        // exponen el mismo catálogo que PLDMGR. Si GitHub ha devuelto 403/429 recientemente,
        // no repetimos la consulta: usamos caché/local y dejamos pasar el periodo de enfriamiento.
        if (GitHubRateLimitActive())
        {
            LogService.Write("GitHub en periodo de enfriamiento por rate limit; se omite la consulta remota.", "WARN");
            return null;
        }
    ...
    private bool GitHubRateLimitActive()
    {
        try
        {
            var path = Path.Combine(_cacheDir, GitHubRateLimitCacheFile);
            if (!File.Exists(path)) return false;
            if (!DateTimeOffset.TryParse(File.ReadAllText(path), out var until)) return false;
    ...
    private void MarkGitHubRateLimit()
    {
        try
        {
            var path = Path.Combine(_cacheDir, GitHubRateLimitCacheFile);
            File.WriteAllText(path, (DateTimeOffset.UtcNow + GitHubRateLimitBackoff).ToString("O"));
        }
        catch { }
    }
