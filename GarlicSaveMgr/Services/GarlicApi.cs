...
public sealed class GarlicApi : IDisposable
{
    ...
    {
        var root = await GetJsonAsync("/users", 5, ct);
        return ExtractArray(root, "users");
    }

    public async Task<List<string>> UserIdsAsync(CancellationToken ct = default)
    {
        try
        {
            var profiles = await AccountIdsAsync(ct);
            var ids = ExtractUserIds(profiles);
            if (ids.Count > 0) return ids;
        }
        catch (GarlicException)
        {
            // Endpoint unavailable on this Garlic build; use the documented fallback.
        }

        var fallback = await UsersAsync(ct);
        return ExtractUserIds(fallback);
    }

    private static List<string> ExtractUserIds(IEnumerable<JsonElement> profiles)
        => profiles
            .Select(ProfileImportValue)
